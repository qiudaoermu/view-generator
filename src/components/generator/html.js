...
const tags = {
  'el-image': el => {
    ...
    const style = el['style'] ? `:style='${JSON.stringify(el['style'])}'` : ''
    let child = buildElButtonChild(el)

    if (child) child = `\n${child}\n` // 换行
    return `<${tag} ${style} ${type} ${icon} ${round} ${size} ${plain} ${disabled} ${circle}>${child}</${tag}>`
  },
  'el-button': el => {
    const {
      tag, disabled
  ...
  'el-switch': el => {
    const { tag, disabled, vModel } = attrBuilder(el)
    const style = el['active-text'] ? `active-text="${el['active-text']}"` : ''
  ...
  'el-time-picker': el => {
    ...
    const style = el['picker-options'] ? `:picker-options='${JSON.stringify(el['picker-options'])}'` : ''
